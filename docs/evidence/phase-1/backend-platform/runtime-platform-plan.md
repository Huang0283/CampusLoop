...
## 1. 版本基线（候选锁定目标；以 Phase 2 交付的 backend/requirements.txt 为准）

> 说明：下表为 Phase 2 将锁定的**具体版本**（与 BP2-06 交付的 `backend/requirements.txt` 完全一致）。**Phase 1 分支不含该文件**——`backend/requirements.txt` 随 Phase 2 代码合入仓库后才存在，届时以该文件为唯一事实来源；如需调整版本，先改 requirements.txt 再同步本表。

| 组件 | 锁定版本 | 说明 |
|---|---|---|
| Python | 3.12 | 容器与 CI 一致 |
| FastAPI | 0.115.6 | 含 TestClient 所需依赖 |
| uvicorn[standard] | 0.32.1 | ASGI 服务器 |
| SQLAlchemy | 2.0.36 | 声明式模型 |
| Alembic | 1.14.0 | 迁移 |
| psycopg | 3.2.3 (binary) | PG 驱动 |
| pgvector Python | 0.3.6 | embedding 列类型 |
| pydantic / pydantic-settings | 2.10.3 / 2.6.1 | 校验与配置 |
| redis-py | 5.2.1 | 健康检查/缓存 |
| httpx | 0.28.1 | TestClient 底层传输 |
| PostgreSQL | 16（pgvector/pgvector:pg16 镜像） | 含 vector 扩展 |
| Redis | 7-alpine | — |
| MinIO | minio/minio:latest（Compose 固定写法） | S3 协议 |
| Node（CI 前端） | 22 | 与前端 Vite 8 匹配 |
| ruff | 0.8.4 | 格式 + 静态检查 |
| pytest | 8.3.4 | 单元/接口测试 |
...
- [ ] 非作者可据此在干净机器上复现环境 —— **待 Phase 2 验证**：BP2-09 在干净环境 Compose 实测全绿、CI 通过后由验证人勾选；Phase 1 仅为方案盘点，不声称已复现
