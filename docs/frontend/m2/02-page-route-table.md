...
路由表的“访问角色”只描述页面能否打开，页面内动作必须单独记录：

- 公开浏览：`/`、`/market`、`/product/:id`、`/wanted`、`/wanted/:id`。游客可以查看列表、详情、搜索、筛选、排序、分页和返回列表。
- 登录动作：收藏、联系卖家/发布者、举报、发布商品、发布求购和查看个性化匹配。游客点击时保存来源路径并进入 `/login`，不能因为浏览页面本身弹出登录提示。
- 受保护页面：`/profile`、`/favorites`、`/my-products`、`/publish`、`/wanted/publish`、`/wanted/matches`、聊天、交易、通知和管理后台。
- `/admin` 继续执行登录校验和管理员角色校验；普通学生进入 `/403`。
- `/product/:id/edit` 与 `/wanted/:id/edit` 只有在路由实际实现后才能标记为可验收，不能只写在映射表中。
